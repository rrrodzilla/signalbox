...
`down` and `restart` also refuse while any runs are in flight, naming every blocking run in one refusal. Pass `--force` to override the guard. A run stops blocking when its event trail has been quiet longer than `SIGNALBOX_IN_FLIGHT_WINDOW` seconds (2400 by default) and it has no live model subprocess attributable to its worktree beneath the owned engine. Set that environment variable to tune the quiet window. A retained worktree alone is deliberately not evidence that a run is in flight: halted runs keep their worktrees for diagnosis without preventing teardown.
