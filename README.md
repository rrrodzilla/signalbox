...
The launcher API is `bin/run.sh <issue> [--phase pipeline|plan|implement|review]`; the default `pipeline` phase runs the whole per-feature path (plan → implement → review → **promote**). At launch it renders all five child configs from `templates/` into `runs/issue-<n>/`, substituting that run's engine suffix and leased approval port. The phase-runner launches children from those configs, propagates the run slug, and watches **disk artifacts** in the same run root (never engine claims) for the terminal condition: fresh `plan.json`, fresh `state/gate.json`, or, for review, `results/CR.md` or `state/pending.json`. When parallel docs-sync is still in flight at an unparked review terminal, the runner hands its exact child PID — paired with that process's `/proc/<pid>/stat` start time, so the reaper can tell the engine from a later process that inherits the number — to the detached, 960-second deferred reaper in `bin/engine-reaper.sh`, so sync can finish after the pipeline advances rather than being killed at the terminal; otherwise it stops the child PID gracefully itself so the event trail flushes. The foreground launcher records the outer engine PID in `state/engine.pid`; each live child phase records `state/phase-<phase>.pid` and removes it after reaping. Selecting `plan`, `implement`, or `review` launches one phase through the same isolation machinery. The direct `SIGNALBOX_ISSUE=<n> emergent --config pipeline.toml` path uses the installed unsuffixed config and remains the single-run form.
...
The installed harness is a rendered copy, not a symlink, so any harness-source merge leaves it stale. Before the next pipeline run after this merge, signalbox's own `.claude/emergent/` must be reinstalled, carrying `state/assessments.jsonl` and `state/readiness.json` into the fresh install. That reinstall is also what invokes `bin/sink-service.sh ensure` to install and start the shared sink service.
