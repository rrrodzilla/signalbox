...
Step 3 runs plan → implement → review → promote with Fable operating the phase seams. Releases remain human-owned, and the situational gate can park a run for human approval when its earned readiness is below the assessed floor. Run the launcher once per issue and N issues can proceed side by side in the same checkout: each gets its own run directory, engines, ports, logs, and PID files. `bin/run.sh --list` shows every recorded run and whether its recorded engine PID is alive. The direct `SIGNALBOX_ISSUE=42 emergent --config ~/code/my-repo/.claude/emergent/pipeline.toml` form remains available for one run at a time. Details per phase below.
...
## Watching runs (`bin/watch.sh`)

Every topology carries a `watchtower` sse-sink streaming its interesting topics (phase seams and operator verdicts, shard builds and review rounds, gate decisions, researcher doc writes). `bin/watch.sh [harness-dir]` serves one live dashboard for the whole harness: it discovers `runs/*/`, renders a separate phase rail, artifact table, and log table for each run, retains an honest `single-run` view when root artifacts exist, and merges every run's labeled events through one origin (no CORS). A port lease released by a finished run can be re-leased later, so a port claimed by more than one run's TOML goes to the live run and a dead run gets no stream — liveness means the run's `state/engine.pid` marker still names its launch PID *and* that process's start identity — `<boot epoch>:<start ticks>` straight out of `/proc`, recorded by `bin/run.sh` in `launch.json` as `start_id` — is exactly the one recorded at spawn, so a PID recycled after a SIGKILL or reboot can't resurrect a finished run no matter how soon it is reused. Each SSE response also names the owner it resolved at connect time, and the page freezes that identity per connection, so a handoff between polls can't label one run's buffered events (or promote state) with another's.

The dashboard assumes no port numbers. Installation reserves each repo's static six-port block in `~/.local/share/signalbox/ports.json`, preserving the direct single-run path. The launcher then leases a distinct six-port block for each concurrent run from `~/.local/share/signalbox/leases.json`, avoiding every reserved repo block and live lease; it releases the lease on exit, and stale leases are reaped after a crash by the owner's recorded start identity (`<boot epoch>:<start ticks>` from `/proc`, not the PID alone) — so a reboot or a recycled PID can neither keep a dead lease alive nor reject a valid launch. The dashboard re-discovers each run's rendered TOMLs and unions their watchtower ports. Its own page port is `WATCH_PORT` if set, else 8099, else an ephemeral port — the URL actually bound is always printed. Artifact rails work even for engines started without watchtowers; streams light up whenever a sink-equipped engine is running.
