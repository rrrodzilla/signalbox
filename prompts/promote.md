...
2. The integration worktree is clean (`git status --porcelain` empty) and the feature branch contains commits over the base branch. Because docs-sync now runs concurrently with the review seam rather than before it, this is the first clean-worktree check guaranteed to run after sync finishes: verify it now, when you are about to push, and do not carry forward an earlier observation.
3. The working tree diff `base...feature` touches only files consistent with the plan's shards (`plan.json` in the run root).
4. `state/docs-sync.json` in the run root is fresh (newer than `state/pipeline-review.stamp` in that same run root) with `status: "OK"`. The executor (`bin/promote-exec.sh`, via `bin/docs-sync-wait.sh`) has already blocked on exactly this condition before invoking you, so it should hold when you look. If the evidence is nevertheless absent, stale, not `status: "OK"`, or has a `correlation_id` that disagrees with `results/CR.md`, return NO_GO; do not work around it.
...
7. If the plan's scope_notes flag a genuine release consequence (version bump, tag, publish, or plan-declared breaking change), add a brief comment on the merged PR addressed to the human release step — that is the handoff, not an action you take. Vault-doc maintenance is excluded: `.claude/docs/{ARCHI.md,ARCHI-rules.md,TESTING.md}` was synced by the completed docs-sync step, with its work recorded in `state/docs-sync.json`. The vault is git-excluded, so those changes never appear in the PR diff; their absence there is not evidence that sync did not happen. If a vault doc is still wrong, return NO_GO with the discrepancy named instead of leaving a PR comment that hands the work to the human.
