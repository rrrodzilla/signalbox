...
3. CI: run `gh pr checks <pr> --watch` with a sensible timeout. If checks exist, ALL must pass — any failure is a NO_GO, do not merge, leave the PR open for a human. If the repository has no CI configured (this is tracked as a known gap in some repos), say so explicitly and rely on the already-verified local gate (clippy + nextest ran green in the integration worktree before the review).
...
7. If the plan's scope_notes flag a release-relevant consequence (e.g. a deliberate breaking change needing a version bump), add a brief comment on the merged PR addressed to the human release step — that is the handoff, not an action you take.
