...
- Launch the per-run supervisor as a background task; monitor **disk artifacts**, never notifications or engine claims. For issue `<n>`, the phase evidence is `.claude/emergent/runs/issue-<n>/{plan.json,state/gate.json,results/CR.md,state/pending.json,state/complete.json,state/halted.json}`; the last two are terminal evidence, and a run is complete or halted only when one of them exists. Init evidence remains the `.proposed.md` files in the repo-scoped vault; pipeline narration is in that run's logs.
