...
- Launch the per-run supervisor as a background task; monitor **disk artifacts**, never notifications or engine claims. For issue `<n>`, the phase evidence is `.claude/emergent/runs/issue-<n>/{plan.json,state/gate.json,results/CR.md,state/pending.json,state/complete.json,state/halted.json}`; the last two are terminal evidence, and a run is complete or halted only when one of them exists. `run.sh` deletes both before it starts the engine, so whichever one appears belongs to the launch you are supervising; if you drive an engine some other way (a direct `emergent --config` launch, for instance), remove both yourself before monitoring, or the previous run's terminal will be read as this one's. Init evidence remains the `.proposed.md` files in the repo-scoped vault; pipeline narration is in that run's logs.
