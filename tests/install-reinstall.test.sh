...
# bin/ exec when bin/ was withdrawn writes its launch.json after the snapshot.
# run.sh records the pid only after spawning the engine, so this metadata names
# no process at all and liveness cannot see it — the snapshot comparison has to
# catch the write itself, refuse, and leave the harness exactly as found. The
# installer's post-withdrawal test seam plays the launcher, so the write lands
# deterministically inside the race window rather than by timing luck.
RACE_RUN_DIR="$FIXTURE_DEST/runs/issue-88"
RACE_LAUNCH="$RACE_RUN_DIR/launch.json"
mkdir -p "$RACE_RUN_DIR"
export SIGNALBOX_REINSTALL_POST_WITHDRAW_HOOK="printf '{\"slug\":\"issue-88\",\"phase\":\"pipeline\",\"issue\":88}\n' >'$RACE_LAUNCH'"
printf '\nPOST_WITHDRAWAL_SENTINEL\n' >>"$FIXTURE_DEST/bin/run.sh"
run_subject "$OUT" "$ERR" --reinstall
unset SIGNALBOX_REINSTALL_POST_WITHDRAW_HOOK
STAGED_LEFT=0
compgen -G "$FIXTURE_DEST/.bin.reinstalling.*" >/dev/null && STAGED_LEFT=1
OK=1
if [ "$RUN_STATUS" -eq 1 ] \
...
mkdir -p "$FIXTURE_DEST/runs/issue-99"
printf '{"slug":"issue-99","phase":"pipeline","issue":99}\n' \
    >"$FIXTURE_DEST/runs/issue-99/launch.json"
cp "$FIXTURE_DEST/runs/issue-99/launch.json" "$FIXTURE_HOME/expected-issue-99.json"
run_subject "$OUT" "$ERR" --reinstall
OK=1
if [ "$RUN_STATUS" -eq 0 ] \
    && [ -d "$FIXTURE_DEST/bin" ] \
    && cmp -s "$FIXTURE_DEST/runs/issue-99/launch.json" "$FIXTURE_HOME/expected-issue-99.json" \
    && ! grep -q 'POST_WITHDRAWAL_SENTINEL' "$FIXTURE_DEST/bin/run.sh"; then
    OK=0
fi
report_case "freshly written preexisting dead metadata does not block a refresh" "$OK" \
    "status=$RUN_STATUS stderr=$(head -c 300 "$ERR")"
